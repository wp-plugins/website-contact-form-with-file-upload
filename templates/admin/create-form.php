...
echo '<p><a class="button" href="'.$url_cancel.'">'.__('&laquo; Existing forms', 'nm-webcontact').'</a></p>';
?>
...
<div id="nmcontact-form-generator">
	...
	<div id="formbox-2">
		...
		foreach ( $types as $type => $data ) {
			...
			$for = (isset($data['for'])) ? $data['for'] : '' ;
			
			echo '<li class="input-type-item '.$for.'" data-inputtype="' . $type . '" data-for="' . $for . '">';
...
function render_input_settings($settings, $values = '') {
	...
	foreach ( $settings as $meta_type => $data ) {
		...
		$metatype = (isset($values [$meta_type])) ? $values [$meta_type] : '' ;
		
		if ($values)
			$setting_html .= '<td class="table-column-input" data-type="' . $data ['type'] . '" data-name="' . $meta_type . '">' . render_input_types ( $data ['type'], $meta_type, $metatype, $options, $for ) . '</td>';
		else
			$setting_html .= '<td class="table-column-input" data-type="' . $data ['type'] . '" data-name="' . $meta_type . '">' . render_input_types ( $data ['type'], $meta_type, null, $options, $for ) . '</td>';
