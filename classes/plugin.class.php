...
class NM_WP_ContactForm extends NM_Framwork_V1 {
	...
	*/
	...
	function wpp_textdomain() {
		...
		load_plugin_textdomain('nm_webcontact', false, $locale_dir);		
	}
	/*
	 * =============== NOW do your JOB ===========================
	*/
	...
	*/
	function save_form_meta() {
		...
		if ($res_id) {
			...
			$resp = array (
					'message' => __ ( 'Form added successfully', 'nm_webcontact' ),
					'status' => 'success',
					'form_id' => $res_id
			);
		} else {
			...
			$resp = array (
					'message' => __ ( 'Error while savign form, please try again', 'nm_webcontact' ),
					'status' => 'failed',
					'form_id' => ''
			);
		}
	...
	*/
	function update_form_meta() {
		...
		if ($res_id) {
			...
			$resp = array (
					'message' => __ ( 'Form updated successfully', 'nm_webcontact' ),
					'status' => 'success',
					'form_id' => $form_id
			);
		} else {
			...
			$resp = array (
					'message' => __ ( 'Error while updating form, please try again', 'nm_webcontact' ),
					'status' => 'failed',
					'form_id' => $form_id
			);
		}
	...
	*/
	function save_settings() {
		...
		_e ( 'All options are updated', 'nm_webcontact' );
		die ( 0 );
	}
	...
	*/
	function send_form_data() {
		...
		$message = "<p>" . __ ( 'Following message is being sent by User', 'nm_webcontact' ) . "</p>";
		...
		foreach ( $receiver_emails as $to ) {
			...
			if (wp_mail ( $to, $subject, $message, $headers, $attachments )) {
				...
				$message_sent = ($message_sent == '') ? __ ( 'Message sent successfully', 'nm_webcontact' ) : $message_sent;
				$resp ['status'] = 'success';
				...
				$resp ['status'] = 'error';
				$resp ['message'] = __ ( 'Error: while seding Email', 'nm_webcontact' );
			}
		}
	...
	*/
	function delete_meta() {
		...
		if ($res) {
			...
			_e ( 'Meta deleted successfully', 'nm_webcontact' );
		} else {
			$wpdb->show_errors ();
			$wpdb->print_error ();
		}
