...
class NM_WP_ContactForm_Admin extends NM_WP_ContactForm {
	...
	function add_menu_pages() {
		foreach ( $this->menu_pages as $page ) {
			...
			if ($page ['parent_slug'] == '') {
				...
				$menu = add_menu_page ( __ ( $page ['page_title'] . ' Settings', 'nm_webcontact' ), __ ( $page ['menu_title'] . ' Settings', 'nm_webcontact' ), $page ['cap'], $page ['slug'], array (
						$this,
						$page ['callback'] 
				...
				$menu = add_submenu_page ( $page ['parent_slug'], __ ( $page ['page_title'] . ' Settings', 'nm_webcontact' ), __ ( $page ['menu_title'] . ' Settings', 'nm_webcontact' ), $page ['cap'], $page ['slug'], array (
						$this,
						$page ['callback'] 
				) );
			}
	...
	function display_contact_form_meta_box($form) {
		echo '<p>' . __ ( 'Following files are uploaded:', 'nm_webcontact' ) . '</p>';
		...
		if ($uploaded_files) {
			foreach ( $uploaded_files as $file ) {
				...
				echo '<td><a href="' . $file_url . '" target="_blank">' . __ ( 'Download file', 'nm_webcontact' ) . '</a></td>';
				echo '</tr>';
			}
		}
		echo '</table>';
	...
	function forms_list() {
		echo '<div class="wrap">';
		echo '<h2>' . __ ( 'N-Media Websit Contact Form Setting', 'nm-webcontact' ) . '</h2>';
		echo '<p>' . __ ( 'Here you can create and edit forms. These forms can be used on any pages/posts', 'nm-webcontact' ) . '</p>';
		
		if ( (isset ( $_REQUEST ['form_id'] ) && $_REQUEST ['form_id'] != NULL) || (isset($_REQUEST['action']) && $_REQUEST['action'] == 'new')) {
			$this->load_template ( 'admin/create-form.php' );
		}else{
			$url_add = $this -> nm_plugin_fix_request_uri(array('action'=>'new'));
			echo '<a class="button button-primary" href="'.$url_add.'">'.__('Add Form', 'nm-webcontact').'</a>';
		}
